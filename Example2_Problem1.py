...
for i in range(1,Np+1):
    s.hw[4*i-3]=(varepsilon)*1*tau
    s.hw[4*i-2]=(varepsilon)*1*tau
...
# s.RCI()
s.Margin_Design() # Computes a graph
...
if False:
    ...
    for t in range(s.K,60):
        w={}
        xi_l=beta*random.random()**0.2*(-1)**(random.random()>0.5)
